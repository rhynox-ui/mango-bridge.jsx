...
import { runOpDeposit, initiateOpWithdrawal, getOpWithdrawalStatus, proveOpWithdrawal, finalizeOpWithdrawal } from "./opbridge.js";
...
function MangoLogo({ size = 36 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" className="shrink-0">
      <defs>
        <linearGradient id="mangoBody" x1="10" y1="8" x2="54" y2="52" gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor="#FFD23D" />
          <stop offset="0.55" stopColor="#FF9E2C" />
          <stop offset="1" stopColor="#7FCC2A" />
        </linearGradient>
        <linearGradient id="mangoLeaf" x1="34" y1="4" x2="50" y2="16" gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor="#9FE94B" />
          <stop offset="1" stopColor="#5FAE1E" />
        </linearGradient>
      </defs>
      <rect width="64" height="64" rx="14" fill="#0A0C10" />
      <path
        d="M32 10c9 0 16 8.5 16 20s-8 22-16 22-16-10.5-16-22 7-20 16-20Z"
        fill="url(#mangoBody)"
      />
      <path d="M33 10c1-3 4-5 7-5 0 3-2 5.5-5 6.5-1-0.3-1.5-0.9-2-1.5Z" fill="#5B3A1A" />
      <path d="M35 11c5-1 9 1.5 10 5-4 1.5-9-0.5-10-5Z" fill="url(#mangoLeaf)" />
      <line x1="18" y1="34" x2="46" y2="34" stroke="#10130A" strokeWidth="2" strokeLinecap="round" opacity="0.55" />
      <line x1="24" y1="24" x2="24" y2="34" stroke="#10130A" strokeWidth="2" strokeLinecap="round" opacity="0.55" />
      <line x1="40" y1="24" x2="40" y2="34" stroke="#10130A" strokeWidth="2" strokeLinecap="round" opacity="0.55" />
      <line x1="18" y1="34" x2="24" y2="24" stroke="#10130A" strokeWidth="2" strokeLinecap="round" opacity="0.55" />
      <line x1="46" y1="34" x2="40" y2="24" stroke="#10130A" strokeWidth="2" strokeLinecap="round" opacity="0.55" />
    </svg>
  );
}
...
function BridgeModal({ from, to, amount, asset, fee, etaLabel, received, devFeeAmount, destination, account, onClose, onComplete, onWithdrawalInitiated }) {
  ...
  const steps = kind === "cctp" ? CCTP_STEPS : kind === "op-deposit" ? OP_DEPOSIT_STEPS : kind === "op-withdraw" ? OP_WITHDRAW_INITIATE_STEPS : STEPS;
    ...
    try {
      if (kind === "cctp") {
        ...
        const result = await initiateOpWithdrawal({
          account, amountHuman: amount,
          onStep: (key) => { if (key !== "done") setStepIndex(OP_WITHDRAW_INITIATE_STEP_INDEX[key] ?? 0); },
        });
        setRealBurnHash(result.l2TxHash);
        setStepIndex(steps.length);
        setPhase("withdrawal-initiated");
        onWithdrawalInitiated?.({ l2TxHash: result.l2TxHash, l2Timestamp: result.l2Timestamp, amount, account });
      }
    } catch (err) {
  ...
  const displayHash = realBurnHash || simulatedHash;
        ...
        {phase === "review" && (
          <>
            <div className="flex flex-col gap-2.5 mb-5">
              ...
              <div className="flex items-center justify-between text-[13px]"><span style={{ color: "#5B6472" }}>Estimated time</span><span style={{ color: "#D7DBE2" }}>{kind === "op-withdraw" ? "~1hr to prove, then 7 days to finalize" : etaLabel}</span></div>
            ...
            <button onClick={handleConfirm} className="w-full py-3 rounded-xl font-display font-semibold text-[14.5px]" style={{ background: `linear-gradient(135deg, ${LIME}, ${LIME_DEEP})`, color: "#10130A" }}>
              {kind === "op-withdraw" ? "Start withdrawal" : "Confirm bridge"}
            </button>
          </>
        )}
              ...
              <div className="mt-2 flex flex-col gap-2">
                ...
                {isReal ? (
                  <>
                    ...
                    >
                      View {kind === "op-deposit" ? "L1" : "burn"} tx on {a.name} <ExternalLink size={13} />
                    </a>
                    <a
...
const WITHDRAWAL_STATUS_LABEL = {
  ...
  "waiting-to-finalize": "Proven — waiting out the 7-day challenge period",
...
function WithdrawalRow({ w, onUpdate }) {
  ...
  async function checkStatus() {
    ...
    setError("");
    try {
      const { status, etaSeconds } = await getOpWithdrawalStatus({ l2TxHash: w.l2TxHash, l2Timestamp: w.l2Timestamp });
      onUpdate({ ...w, status, etaSeconds, lastChecked: Date.now() });
    } catch (err) {
  ...
  async function finalize() {
    ...
    setError("");
    try {
      const { finalizeHash } = await finalizeOpWithdrawal({ l2TxHash: w.l2TxHash });
  ...
  return (
    <div className="px-4 py-3.5" style={{ borderTop: "1px solid #1A1E26" }}>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[13.5px] font-medium" style={{ color: "#F2F4F7" }}>{w.amount} ETH — Base → Ethereum</span>
...
export default function MangoBridge() {
  ...
  function handleWithdrawalInitiated({ l2TxHash, l2Timestamp, amount: amt, account: acct }) {
    const entry = { id: Date.now(), l2TxHash, l2Timestamp, amount: amt, account: acct, initiatedAt: Date.now(), status: "waiting-to-prove" };
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex gap-1 p-1 rounded-xl" style={{ background: "#12151B" }}>
              {["bridge", "history", "withdrawals"].map((t) => (
          ...
          {tab === "history" ? (
            ...
            <>
              ...
              >
                {!connected ? "Connect wallet" : onWrongNetwork ? "Switch network to continue" : from === to ? "Choose different chains" : amtNum <= 0 ? "Enter an amount" : insufficient ? "Insufficient balance" : sendToOther && destAddress.trim().length <= 4 ? "Enter destination address" : getTransferKind(from, to, asset.symbol) === "op-withdraw" ? "Start withdrawal" : "Bridge assets"}
              </button>

              <div className="text-center mt-4 text-[11.5px]" style={{ color: "#3A414C" }}>
                Testnet only. Ethereum Sepolia ↔ Base Sepolia is real for USDC (CCTP) and ETH (Base's bridge — withdrawals take 7 real days). All other routes are simulated.
              </div>
