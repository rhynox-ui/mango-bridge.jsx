...
function AssetDropdown({ assetIdx, setAssetIdx, chainId, P }) {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-40 rounded-xl overflow-hidden shadow-2xl" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
          {ASSETS.map((a, i) => (
            <button key={a.symbol} onClick={() => { setAssetIdx(i); setOpen(false); }} className="w-full flex items-center justify-between px-3 py-2.5 text-left">
    ...
    <div className="flex items-center justify-between px-4 py-3.5" style={{ borderTop: isFirst ? "none" : `1px solid ${P.divider}` }}>
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[11px]" style={{ color: P.textMuted }}>{c.short === "BNB" ? "BNB" : "ETH"}</span>
        </div>
      </div>
      <span className="text-[13.5px] font-mono font-medium" style={{ color: P.textPrimary }}>
        {!connected ? "—" : isLoading ? "…" : data ? Number(data.formatted).toFixed(4) : "0.0000"}
      </span>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
      ...
      <div className="px-4 py-3 text-[11px]" style={{ color: P.textMuted, borderTop: `1px solid ${P.divider}` }}>
        Native token balance shown per chain. Token-specific balances (USDC, etc.) show on the Bridge tab.
      </div>
    </div>
  );
}
...
export default function MangoBridge() {
  ...
  return (
    <div className="min-h-screen w-full pb-24" style={{ background: P.bg, fontFamily: "'Inter', sans-serif" }}>
      <style>{`
      ...
      {/* Top nav */}
      <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: `1px solid ${P.divider}` }}>
        <div className="flex items-center gap-2.5">
          <MangoLogo size={32} color={P.textPrimary} />
          <div className="flex flex-col leading-tight">
            <span className="font-display text-[16px] font-semibold tracking-tight" style={{ color: P.textPrimary }}>Mango Bridge</span>
            ...
            <>
              ...
              <div className="rounded-2xl p-4 shadow-sm" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
                ...
                <div className="flex items-center justify-between rounded-xl px-3.5 py-3" style={{ background: P.input, border: `1px solid ${insufficient ? "#D92D20" : P.panelBorder}` }}>
                  ...
                  />
                  <button onClick={setMax} className="text-[10.5px] font-bold px-2 py-1 rounded-md mr-2 shrink-0" style={{ background: `${LIME}1A`, color: LIME_DEEP }}>MAX</button>
              ...
              <div className="rounded-2xl p-4 mt-3 shadow-sm" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
                <div className="flex items-center justify-between mb-2.5">
                  ...
                  <span className="text-[11.5px]" style={{ color: P.textMuted }}>Balance: {fmt(balances?.[to]?.[asset.symbol] ?? 0, asset.decimals)} {asset.symbol}</span>
                </div>
                <div className="flex items-center justify-between mb-3">
