import { NetworkEthereum, NetworkBase, NetworkBinanceSmartChain } from "@web3icons/react";
...
function AssetIcon({ symbol, size = 18 }) {
  ...
  // sanctioned use case, not a guess, so it takes priority over the
  // dynamic lookup below. Falls back to the hand-drawn glyph if the image
  // ever fails to load, via proper React state rather than direct DOM
  // manipulation.
  if (symbol === "USDG") {
    return (
  ...
  return (
    <span className="flex items-center justify-center rounded-full shrink-0 overflow-hidden" style={{ width: size, height: size }}>
      <TokenIcon
        ...
        variant="branded"
        size={size}
        fallback={<HandDrawnAssetGlyph symbol={symbol} size={size} color={color} />}
      />
    </span>
  );
}
