        ...
        {phase === "review" && (
          <>
            ...
            <div className="flex items-start gap-2 mb-4 px-3 py-2.5 rounded-lg text-[12px]" style={{ background: isReal ? `${LIME}14` : "#1C212A", border: `1px solid ${isReal ? LIME + "40" : "#262C36"}`, color: isReal ? LIME : "#8B95A1" }}>
              ...
              {kind === "wormhole" && `Real ${networkMode === "mainnet" ? "" : "testnet "}transfer via Wormhole. You'll receive Wormhole-wrapped ETH on ${CHAINS.bnb.name} — not native BNB or any other app's wrapped ETH. This is the newest, least battle-tested integration here — start very small.`}
          ...
          <div className="flex flex-col gap-3">
            ...
            {isReal && realBurnHash && (
              <div className="flex flex-col gap-2 px-3 py-2.5 rounded-lg text-[12px]" style={{ background: `${LIME}14`, border: `1px solid ${LIME}40`, color: LIME }}>
                ...
                {kind === "wormhole" && <span>Your ETH is locked on {CHAINS.ethereum.name} — nothing is lost. Wait a bit for the guardians to finish, then tap Resume below.</span>}
...
function MainnetConfirmModal({ onConfirm, onCancel, P }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(4,5,7,0.75)", backdropFilter: "blur(4px)" }}>
      <div className="w-full max-w-sm rounded-2xl p-5" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
        <div className="flex items-center gap-2 mb-3">
          <AlertTriangle size={18} color={LIME_DEEP} />
          <span className="font-display text-[16px] font-semibold" style={{ color: P.textPrimary }}>Switch to Mainnet?</span>
        </div>
        <div className="text-[13px] leading-relaxed mb-4" style={{ color: P.textSecondary }}>
          Every transfer from here uses real funds on real networks.
        </div>
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-lg text-[13.5px] font-medium" style={{ background: P.input, color: P.textSecondary, border: `1px solid ${P.panelBorder}` }}>
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="flex-1 py-2.5 rounded-lg text-[13.5px] font-semibold"
            style={{ background: LIME, color: "#10130A" }}
          >
            Switch to Mainnet
          </button>
        </div>
...
export default function MangoBridge() {
  ...
  return (
    <div className="min-h-screen w-full pb-24 relative" style={{ background: P.bg, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-6 py-4 relative" style={{ borderBottom: `1px solid ${P.divider}`, zIndex: 1 }}>
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col leading-tight">
            ...
            <span className="text-[10px] font-medium tracking-wide uppercase" style={{ color: networkMode === "mainnet" ? LIME_DEEP : P.textMuted }}>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={{
              background: networkMode === "mainnet" ? LIME : P.panel,
              color: networkMode === "mainnet" ? "#10130A" : P.textSecondary,
              border: `1px solid ${networkMode === "mainnet" ? LIME : P.panelBorder}`,
            }}
          >
      ...
      {networkMode === "mainnet" && (
        <div className="flex items-center justify-center gap-2 px-6 py-1.5" style={{ background: `${LIME}1A`, borderBottom: `1px solid ${LIME}40` }}>
          <span className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: LIME_DEEP }}>Mainnet — real funds</span>
        </div>
      )}
          ...
          {tab === "history" ? (
            ...
            <>
              ...
              <div className="text-center mt-4 text-[11.5px]" style={{ color: P.textMuted }}>
                {networkMode === "mainnet"
                  ? "Mainnet. Real: Ethereum↔Base (USDC/ETH), Ethereum↔Robinhood Chain (ETH/USDC), Ethereum→BNB (ETH via Wormhole, wrapped). Withdrawals take ~7 real days. BNB→Ethereum and all other pairs are simulated."
