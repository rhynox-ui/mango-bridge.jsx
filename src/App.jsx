...
function getTransferKind(fromKey, toKey, assetSymbol) {
  ...
  if (fromKey === "base" && toKey === "ethereum" && assetSymbol === "ETH" && canRelayHandle(fromKey, toKey, assetSymbol, assetSymbol)) return "relay";
  if (fromKey === "base" && toKey === "ethereum" && assetSymbol === "ETH") return "op-withdraw";
  if (fromKey === "robinhood" && toKey === "ethereum" && (assetSymbol === "ETH" || assetSymbol === "USDC") && canRelayHandle(fromKey, toKey, assetSymbol, assetSymbol)) return "relay";
  if (fromKey === "robinhood" && toKey === "ethereum" && (assetSymbol === "ETH" || assetSymbol === "USDC")) return "arb-withdraw";
  if (fromKey === "bnb" && toKey === "ethereum" && assetSymbol === "ETH") return "wormhole-reverse";
  if (canRelayHandle(fromKey, toKey, assetSymbol, assetSymbol)) return "relay";
  return "simulated";
}
        ...
        {phase === "review" && (
          <>
            ...
            <div className="flex items-start gap-2 mb-4 px-3 py-2.5 rounded-lg text-[12px]" style={{ background: isReal ? `${LIME}14` : "#1C212A", border: `1px solid ${isReal ? LIME + "40" : "#262C36"}`, color: isReal ? LIME : "#8B95A1" }}>
              ...
              {kind === "relay" && "Route uses Relay's solver network to enable fast, non-custodial cross-chain transfers."}
...
export default function MangoBridge() {
  ...
  function handleAssetChange(idx) { setAssetIdx(idx); setAmount(""); }
  ...
  const gasReserve = GAS_RESERVE[asset] ?? 0;
            ...
            <>
              ...
              <div className="rounded-2xl p-4 shadow-sm" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
                ...
                <div className="flex items-center justify-between rounded-xl px-3.5 py-3" style={{ background: P.input, border: `1px solid ${insufficient ? "#D92D20" : P.panelBorder}` }}>
                  ...
                  <AssetDropdown assetIdx={assetIdx} setAssetIdx={handleAssetChange} chainId={from} P={P} />
                </div>
              ...
              <div className="rounded-2xl p-4 mt-3 shadow-sm" style={{ background: P.panel, border: `1px solid ${P.panelBorder}` }}>
                ...
                <div className="flex items-center justify-between rounded-xl px-3.5 py-3" style={{ background: P.input, border: `1px solid ${P.panelBorder}` }}>
                  ...
                  <AssetDropdown assetIdx={assetIdx} setAssetIdx={handleAssetChange} chainId={to} P={P} />
                </div>
              </div>
              ...
              <div className="text-center mt-4 text-[11.5px]" style={{ color: P.textMuted }}>
                Real: Ethereum↔Base (USDC/ETH), Ethereum↔Robinhood Chain (ETH/USDC), Ethereum↔BNB (ETH via Wormhole, wrapped). BNB, USDT, USDC, USDG moving between other chains route via Relay Protocol (different trust model — a solver network, not an audited canonical bridge). Withdrawals take ~7 real days. Cross-asset swaps (e.g. ETH→USDC) aren't supported yet — same asset, different chain only.
              </div>
