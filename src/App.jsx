...
import { getRelayQuote, executeRelayQuote, canRelayHandle, MAINNET_CHAIN_IDS, ASSET_ONCHAIN_DECIMALS } from "./relaybridge.js";
    ...
    try {
      if (kind === "cctp") {
        ...
        const quote = await getRelayQuote({
          ...
          amountBaseUnits, userAddress: account,
        });
        ...
        setRealMintHash(result.txHashes[result.txHashes.length - 1]);
        setStepIndex(steps.length);
        setPhase("done");
        ...
        {phase === "review" && (
          <>
            ...
            <div className="flex items-start gap-2 mb-4 px-3 py-2.5 rounded-lg text-[12px]" style={{ background: isReal ? `${LIME}14` : "#1C212A", border: `1px solid ${isReal ? LIME + "40" : "#262C36"}`, color: isReal ? LIME : "#8B95A1" }}>
              ...
              {kind === "relay" && `Real transfer via Relay Protocol. Different trust model than the other routes here — you're trusting Relay's solver network to fulfill, not an audited canonical bridge. Non-custodial and used by major wallets, but genuinely newer to this app — start very small.`}
