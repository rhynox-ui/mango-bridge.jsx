import { NetworkEthereum, NetworkBase, NetworkBinanceSmartChain, TokenETH, TokenUSDC, TokenUSDT, TokenWBTC, TokenBNB } from "@web3icons/react";
...
function ChainIcon({ id, size }) {
  ...
  // Robinhood Chain isn't in @web3icons/react's set (confirmed by checking,
  // not assumed) — keeping the hand-drawn version here rather than guess at
  // an import name that might not exist and break the build.
...
function AssetIcon({ symbol, size = 18 }) {
  ...
  const BRANDED = { USDC: TokenUSDC, ETH: TokenETH, USDT: TokenUSDT, BNB: TokenBNB, WBTC: TokenWBTC };
  if (BRANDED[symbol]) {
    const Icon = BRANDED[symbol];
    return (
  ...
  // USDG isn't confirmed present in @web3icons/react's set — hand-drawn
  // rather than guess at an import name that might not exist and break
  // the build.
  let glyph;
  if (symbol === "USDG") {
    glyph = (
...
function getTransferKind(fromKey, toKey, fromAssetSymbol, toAssetSymbol) {
  ...
  // Relay is the only route that supports different assets on each side
  // (a genuine swap, not just a same-asset cross-chain transfer) — every
  // route above this point only ever applies when sameAsset is true.
  if (canRelayHandle(fromKey, toKey, fromAssetSymbol, toAssetSymbol)) return "relay";
  return "simulated";
}
...
export default function MangoBridge() {
  ...
  const isNativeAsset = fromAsset.symbol === "ETH" || (fromAsset.symbol === "BNB" && from === "bnb");
  ...
  const isNativeAssetTo = toAsset.symbol === "ETH" || (toAsset.symbol === "BNB" && to === "bnb");
  ...
  const GAS_RESERVE = { ETH: 0.0004, BNB: 0.001 };
        ...
        <div className="w-full max-w-[420px]">
          ...
          {tab === "history" ? (
            ...
            <>
              ...
              >
                {!connected ? "Connect wallet" : onWrongNetwork ? "Switch network to continue" : from === to ? "Choose different chains" : amtNum <= 0 ? "Enter an amount" : insufficient ? "Insufficient balance" : sendToOther && destAddress.trim().length <= 4 ? "Enter destination address" : ["op-withdraw", "arb-withdraw"].includes(getTransferKind(from, to, fromAsset.symbol, toAsset.symbol)) ? "Start withdrawal" : isCrossAsset ? "Swap assets" : "Bridge assets"}
              </button>

              <div className="text-center mt-4 text-[11.5px]" style={{ color: P.textMuted }}>
                Real: Ethereum↔Base (USDC/ETH), Ethereum↔Robinhood Chain (ETH/USDC), Ethereum↔BNB (ETH via Wormhole, wrapped). BNB, USDT, USDC, USDG moving between chains — and swaps between different assets — route via Relay Protocol where a verified contract address exists on both sides (different trust model — a solver network, not an audited canonical bridge). Withdrawals take ~7 real days. Anything without a verified address stays simulated rather than guessed.
              </div>
