import { NetworkEthereum, NetworkBase, NetworkBinanceSmartChain, NetworkStablechain, TokenETH, TokenUSDC, TokenUSDT, TokenBNB, TokenWBTC } from "@web3icons/react";
...
function ChainIcon({ id, size }) {
  ...
  if (id === "stable") return <NetworkStablechain variant="branded" size={s} />;
...
function AssetIcon({ symbol, size = 18 }) {
  ...
  // examples and confirmed individual pages on web3icons.io (TokenETH,
  // TokenUSDC, TokenUSDT, TokenBNB, TokenWBTC — WBTC confirmed via its own
  // dedicated page there, not assumed) — no dynamic lookup. The dynamic
  // /dynamic entry point requires a <Suspense> boundary to function at all,
  // which this app didn't have, and it crashed the app on every load as a
  // result — a real production issue, not a hypothetical one. Static
  // imports have no such requirement. USDT0 and anything else not in this
  // confirmed set use the hand-drawn fallback instead of risking either
  // failure mode again.
  const STATIC_ICONS = { ETH: TokenETH, USDC: TokenUSDC, USDT: TokenUSDT, BNB: TokenBNB, WBTC: TokenWBTC };
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        await getRelayQuote({
          ...
          amountBaseUnits, userAddress: address,
        });
