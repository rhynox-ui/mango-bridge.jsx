...
import { getNetworkMode, setNetworkMode, isMainnet, getWagmiChain } from "./networkMode.js";
...
const ARB_ERC20_DEPOSIT_STEPS = [
  { key: "build", label: "Preparing deposit" },
  { key: "approve", label: "Approving USDC spend" },
  { key: "deposit", label: "Depositing on Ethereum Sepolia" },
  { key: "confirm", label: "Crediting on Robinhood Chain Testnet" },
];
const ARB_ERC20_DEPOSIT_STEP_INDEX = { build: 0, approve: 1, deposit: 2, confirm: 3 };
...
function getTransferKind(fromKey, toKey, assetSymbol) {
  ...
  if (fromKey === "ethereum" && toKey === "robinhood" && (assetSymbol === "ETH" || assetSymbol === "USDC")) return "arb-deposit";
  if (fromKey === "robinhood" && toKey === "ethereum" && (assetSymbol === "ETH" || assetSymbol === "USDC")) return "arb-withdraw";
...
function BridgeModal({ from, to, amount, asset, fee, etaLabel, received, devFeeAmount, destination, account, onClose, onComplete, onWithdrawalInitiated }) {
  ...
  const steps = kind === "cctp" ? CCTP_STEPS
    ...
    : kind === "arb-deposit" ? (asset === "USDC" ? ARB_ERC20_DEPOSIT_STEPS : ARB_DEPOSIT_STEPS)
...
function MainnetConfirmModal({ onConfirm, onCancel, P }) {
  const [typed, setTyped] = useState("");
  const canConfirm = typed.trim().toUpperCase() === "MAINNET";
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(4,5,7,0.75)", backdropFilter: "blur(4px)" }}>
      <div className="w-full max-w-sm rounded-2xl p-5" style={{ background: "#14171D", border: "1px solid #4A1E1E" }}>
        <div className="flex items-center gap-2 mb-3">
          <AlertTriangle size={18} color="#E5726B" />
          <span className="font-display text-[16px] font-semibold" style={{ color: "#F2F4F7" }}>Switching to Mainnet</span>
        </div>
        <div className="text-[13px] leading-relaxed mb-4" style={{ color: "#B6BEC9" }}>
          Every transfer from here on uses <b>real funds on real networks</b>. There is no undo, no testnet faucet, and no simulated fallback. Only proceed if you've verified the contract addresses and tested this specific route thoroughly.
        </div>
        <div className="text-[12px] mb-2" style={{ color: "#8B95A1" }}>Type MAINNET to confirm:</div>
        <input
          value={typed}
          onChange={(e) => setTyped(e.target.value)}
          placeholder="MAINNET"
          className="w-full px-3 py-2.5 rounded-lg text-[13px] font-mono mb-4"
          style={{ background: "#0E1116", border: "1px solid #262C36", color: "#F2F4F7" }}
        />
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-lg text-[13.5px] font-medium" style={{ background: "#1C212A", color: "#B6BEC9", border: "1px solid #262C36" }}>
            Cancel
          </button>
          <button
            onClick={onConfirm}
            ...
            style={{ background: canConfirm ? "#E5726B" : "#3A2020", color: canConfirm ? "#1A0808" : "#6B4444", cursor: canConfirm ? "pointer" : "not-allowed" }}
          >
            Switch to Mainnet
          </button>
        </div>
...
export default function MangoBridge() {
  ...
  const [networkMode, setNetworkModeState] = useState(getNetworkMode());
  ...
  function requestSwitchToMainnet() {
    setShowMainnetConfirm(true);
  }
  function confirmSwitchToMainnet() {
    setNetworkMode("mainnet");
    setNetworkModeState("mainnet");
    setShowMainnetConfirm(false);
    if (connected) disconnect(); // force a clean reconnect on the new network context
  }
  function switchToTestnet() {
    setNetworkMode("testnet");
    setNetworkModeState("testnet");
  ...
  return (
    <div className="min-h-screen w-full pb-24 relative" style={{ background: P.bg, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-6 py-4 relative" style={{ borderBottom: `1px solid ${P.divider}`, zIndex: 1 }}>
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col leading-tight">
            ...
            <span className="text-[10px] font-medium tracking-wide uppercase" style={{ color: networkMode === "mainnet" ? "#E5726B" : P.textMuted }}>
              {networkMode === "mainnet" ? "Mainnet — Real Funds" : "Testnet"}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => (networkMode === "mainnet" ? switchToTestnet() : requestSwitchToMainnet())}
            className="px-3 py-1.5 rounded-full text-[11px] font-bold"
            style={{
              background: networkMode === "mainnet" ? "#E5726B" : P.panel,
              color: networkMode === "mainnet" ? "#1A0808" : P.textSecondary,
              border: `1px solid ${networkMode === "mainnet" ? "#E5726B" : P.panelBorder}`,
            }}
          >
            {networkMode === "mainnet" ? "MAINNET" : "Go Mainnet"}
          </button>
      ...
      {networkMode === "mainnet" && (
        <div className="flex items-center justify-center gap-2 px-6 py-2" style={{ background: "#E5726B", color: "#1A0808" }}>
          <AlertTriangle size={13} />
          <span className="text-[12px] font-bold uppercase tracking-wide">Mainnet mode — every transfer here uses real funds</span>
        </div>
      )}
      ...
      {showMainnetConfirm && (
        <MainnetConfirmModal
          onConfirm={confirmSwitchToMainnet}
          onCancel={() => setShowMainnetConfirm(false)}
          P={P}
        />
      )}
    </div>
