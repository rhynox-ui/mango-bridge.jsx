...
import { runWormholeTransfer } from "./wormholebridge.js";
    ...
    try {
      if (kind === "cctp") {
        ...
        const result = await runWormholeTransfer({
          amountHuman: amount,
          onStep: (key) => { if (key !== "done") setStepIndex(WORMHOLE_STEP_INDEX[key] ?? 0); },
        });
    ...
    <div className="min-h-screen w-full" style={{ background: "#0A0C10", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid #15181E" }}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowProtocols(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12.5px] font-medium" style={{ background: "#141A24", border: "1px solid #232B38", color: "#B6BEC9" }}>
            ...
            <>
              ...
              {detailsOpen && (
                <div className="mt-2 px-4 py-3 rounded-xl flex flex-col gap-2" style={{ background: "#0E1116", border: "1px solid #1E232B" }}>
                  ...
                  <div className="flex items-center justify-between text-[12.5px]"><span style={{ color: "#5B6472" }}>Protocol fee (1%)</span><span className="font-mono" style={{ color: "#D7DBE2" }}>{fmt(devFeeAmount, asset.decimals)} {asset.symbol}</span></div>
                </div>
              )}
      ...
      {showProtocols && <ProtocolsModal onClose={() => setShowProtocols(false)} />}
    </div>
  );
}
