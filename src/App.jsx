...
export default function MangoBridge() {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        await getRelayQuote({
          ...
          recipientAddress: sendToOther ? destAddress : (isFromSolana ? address : activeAccount),
        });
  ...
  const canBridge = amtNum > 0 && from !== to && !insufficient && !onWrongNetwork && !needsEvmAddressForSolanaSource && (!sendToOther || isValidDestinationAddress(destAddress, CHAINS[to]?.isSolana));
        ...
        <div className="w-full max-w-[420px]">
          ...
          {tab === "history" ? (
            ...
            <>
              ...
              >
                {!connected ? "Connect wallet" : onWrongNetwork ? "Switch network to continue" : from === to ? "Choose different chains" : amtNum <= 0 ? "Enter an amount" : insufficient ? "Insufficient balance" : needsEvmAddressForSolanaSource ? "Connect an EVM wallet to receive on this chain" : sendToOther && !destAddress.trim() ? "Enter destination address" : sendToOther && !isValidDestinationAddress(destAddress, CHAINS[to]?.isSolana) ? `Invalid ${CHAINS[to].name} address` : routeUnavailable ? "No route available for this trade" : routeChecking ? "Checking route…" : ["op-withdraw", "arb-withdraw"].includes(kind) ? "Start withdrawal" : isCrossAsset ? "Swap assets" : "Bridge assets"}
              </button>
              {routeUnavailable && (
