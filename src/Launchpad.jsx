...
import { launchToken, getRealLaunches, buyTokenReal, sellTokenReal, getTokenBalance, ROBINHOOD_CHAIN_ID } from "./launchpad-contracts.js";
...
function TokenAvatar({ name, hue, address, size = 38 }) {
  const logoUrl = address ? TOKEN_LOGOS[address.toLowerCase()] : null;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 p-3" style={{ flex: 1.4, minWidth: 0 }}>
        <TokenAvatar name={token.name} hue={token.hue} address={token.tokenAddress} />
    ...
    <div>
      ...
      <div className="rounded-2xl p-4 mb-3" style={{ background: P.panel, border: `1px solid ${P.panelBorder}`, boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
        <div className="flex items-center gap-3 mb-3">
          <TokenAvatar name={token.name} hue={token.hue} address={token.tokenAddress} size={52} />
          <div className="min-w-0">
