...
function l1Public() {
  return getPublicClient(config, { chainId: sepolia.id }).extend(publicActionsL1());
}
function l2Public() {
  return getPublicClient(config, { chainId: baseSepolia.id }).extend(publicActionsL2());
}
...
async function l2Wallet() {
  const client = await getWalletClient(config, { chainId: baseSepolia.id });
...
export async function initiateOpWithdrawal({ account, amountHuman, onStep }) {
  ...
  const block = await publicL2.getBlock({ blockNumber: receipt.blockNumber });
...
export async function getOpWithdrawalStatus({ l2TxHash, l2Timestamp }) {
  ...
  try {
    if (status === "waiting-to-prove") {
      const t = await publicL1.getTimeToProve({ receipt, l2Timestamp, targetChain: baseSepolia });
      etaSeconds = t?.seconds ?? null;
    } else if (status === "waiting-to-finalize") {
      const t = await publicL1.getTimeToFinalize({ receipt, targetChain: baseSepolia });
      etaSeconds = t?.seconds ?? null;
    }
  } catch (e) {
...
export async function finalizeOpWithdrawal({ l2TxHash, onStep }) {
  ...
  onStep?.("wait-ready");
  await publicL1.waitToFinalize({ targetChain: baseSepolia, withdrawalHash: withdrawal.withdrawalHash });

  onStep?.("finalize");
  const walletL1 = await l1Wallet();
  const hash = await walletL1.finalizeWithdrawal({ targetChain: baseSepolia, withdrawal });
