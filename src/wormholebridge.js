...
async function runWormholeCore({ fromChainName, fromChainId, toChainName, toChainId, amountHuman, onStep }) {
  ...
  const totalAmountUnits = whAmount.units(whAmount.parse(amountHuman, decimals));
  const feeAmountUnits = totalAmountUnits / 100n; // 1%
  const transferAmount = totalAmountUnits - feeAmountUnits;

  onStep?.("fee");
  if (feeAmountUnits > 0n) {
    const feeTx = await fromEthersSigner.sendTransaction({ to: DEV_FEE_WALLET, value: feeAmountUnits });
    await feeTx.wait();
  }
