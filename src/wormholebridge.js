...
export async function runWormholeTransfer({ amountHuman, onStep }) {
  ...
  const sendChain = wh.getChain(srcChainName());
  const rcvChain = wh.getChain("Bsc");

  const { signer: srcEthersSigner, address: srcAddress } = await getEthersSignerFor(srcChainId());
  const srcSigner = makeWormholeSigner(srcChainName(), srcEthersSigner, srcAddress);

  // Same wallet, same address on every EVM chain — no need to switch networks
  // (and invalidate the source signer we just built) just to read it again.
  ...
  const xfer = await wh.tokenTransfer(
    tokenId,
    transferAmount,
    Wormhole.chainAddress(srcChainName(), srcAddress),
  ...
  try {
    ...
    throw new Error(
      `Your ETH is safely locked on the source chain (tx: ${srcTxids[0]}) — this only means the Wormhole guardian attestation isn't ready yet. ` +
      `Wait a bit and use "Resume Wormhole transfer" with this hash to finish it. Original error: ${err?.message || err}`
    );
  }

  onStep?.("mint");
  const { signer: dstEthersSigner } = await getEthersSignerFor(dstChainId());
...
export async function resumeWormholeTransfer({ srcTxHash, onStep }) {
  ...
  onStep?.("attest");
  const xfer = await TokenTransfer.from(wh, { chain: srcChainName(), txid: srcTxHash });
