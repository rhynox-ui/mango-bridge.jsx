...
function ensureClientInitialized() {
  ...
  initPromise = configureDynamicChains().then((chains) => {
    createClient({
...
export async function executeSolanaSourcedTransfer({ solanaAddress, solanaProvider, toChainId, toCurrency, amountBaseUnits, recipient, onProgress }) {
  ...
  const adaptedWallet = adaptSolanaWallet(
    ...
    connection,
    async (transaction) => {
      // Real bug fix, confirmed against the actual installed type
      // definition (adapter.d.ts): this callback must resolve to
      // { signature: TransactionSignature } — a plain object — not the
      // raw signature string that sendRawTransaction itself resolves to.
