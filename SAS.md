...
- mango-mobile added direct, no-API-key EVM fallback providers this site doesn't have (Uniswap V3, Uniswap V4 via Universal Router, SushiSwap V2 — all confirmed deployed on Robinhood Chain directly, closing a real gap this site's own `api/v1/bridge/fallback-supported-chains.js` doesn't have a Uniswap/SushiSwap-direct equivalent for). Worth porting the same providers here if Robinhood Chain fallback coverage becomes a recurring site-side complaint too — not started on this repo, see mango-mobile's own `uniswapV3.js`/`uniswapV4.js`/`sushiswapV2.js` for the exact verified addresses/ABIs if/when this gets built here.
