...
Each native asset (ETH, AVAX, HYPE, XPL, OKB) always routes through Relay. Beyond that: Ink and Unichain additionally have a real OP Stack canonical bridge for ETH (see below); Avalanche, Arbitrum One, and Unichain additionally support USDC via Circle CCTP (verified via `@wormhole-foundation/sdk-base`'s own CCTP constants — an already-installed production dependency of this app — cross-checked against Circle's CREATE2-deterministic CCTP V2 deployment, which uses the identical TokenMessenger/MessageTransmitter address on every supported chain). Abstract, HyperEVM, X Layer, and Plasma have neither yet — Relay only. Chain id / native currency / block explorer for each came directly from `wagmi/chains`' own maintained definitions, not hand-typed.
...
|---|---|---|
| USDC between any two of Ethereum, Base, Avalanche, Arbitrum One, Unichain | [Circle CCTP](https://developers.circle.com/cctp) | Native burn-and-mint. Audited by ChainSecurity and OtterSec. Avalanche/Arbitrum One/Unichain's domain IDs and USDC addresses verified against `@wormhole-foundation/sdk-base`'s own constants, cross-checked byte-for-byte against this app's already-live Ethereum/Base values. |
